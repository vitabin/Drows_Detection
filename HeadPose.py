...
def getHeadTiltAndCoords(size, image_points, frame_height, frame_width):
    focal_length = size[1]
    ...
    (nose_end_point3D, _) = cv2.projectPoints(np.array(
        [(0.0, 1000.0, 0.0)]), rotation_vector, translation_vector, camera_matrix, dist_coeffs)
    ...
    # head_tilt_degree_x = abs(
        # [-180] - np.rad2deg([rotationMatrixToEulerAngles(rotation_matrix)[2]]))
    head_tilt_degree_x = abs(np.rad2deg([rotationMatrixToEulerAngles(rotation_matrix)[2]]))
    ...
    ending_point1 = (int(nose_end_point3D[0][0][0]), int(nose_end_point3D[0][0][1]))

    ending_point_alternate = (ending_point[0], frame_height // 2)
    ending_point_alternate1 = (ending_point1[0], frame_width // 2)
    
    return head_tilt_degree, starting_point, ending_point, ending_point_alternate, ending_point_alternate1, head_tilt_degree_x
